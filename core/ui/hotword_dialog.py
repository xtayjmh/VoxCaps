...
提供 HotwordDialog 类用于显示"添加热词"对话框。
"""
...
class HotwordDialog:
    ...
    def show(
        ...
        Args:
            ...
            title: 对话框标题
            width: 对话框宽度（像素），默认 400

        Returns:
            DialogResult 对象：
            - result.confirmed: 用户是否点击了确定
            - result.data: {'hotword': str}
        """
        # 创建对话框窗口
        ...
        def on_confirm():
            ...
            # 验证输入
            if not value:
                messagebox.showwarning("输入为空", "热词不能为空！")
                return
